...
function ArrowPath({ from, to, color, label, onClick }: {
  ...
  // Shorten end point slightly for arrowhead
  const endX = to.x - (dx / len) * 14
  const endY = to.y - (dy / len) * 14
  // Bezier curve with slight bow
  ...
  return (
    <g className={onClick ? 'cursor-pointer' : ''} onClick={onClick}>
      ...
      {label && (
        <text x={cpx} y={cpy - 6} textAnchor="middle" fontSize={10} fill={color} className="pointer-events-none select-none">
          {label}
        </text>
      )}
    </g>
                  ...
                  className={cn(
                    ...
                    node
                      ...
                      : isEditing && !arrowMode
                        ...
                        : 'border-mat-border/15 bg-transparent'
                  )}
...
function SequenceCard({ sequence, techniques, onDelete }: {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['sequences'] }); setEditing(false); toast.success('Sequence updated.') },
    ...
    <div className={cn(
      ...
      >
        ...
        <div className="flex items-center gap-1 ml-3 shrink-0" onClick={e => e.stopPropagation()}>
          {sequence.coach_assignment_pending ? (
            ...
            <>
              <button
                onClick={() => { setEditing(v => !v); setExpanded(true); setEditName(sequence.name); setEditDesc(sequence.description) }}
                className="p-1.5 text-mat-text-dim hover:text-mat-gold transition-colors"
              >
                <Pencil size={13} />
              </button>
          ...
          <ChevronDown size={13} className={cn('text-mat-text-dim transition-transform ml-1', expanded ? 'rotate-180' : '')} onClick={() => !editing && setExpanded(v => !v)} />
        </div>
      </div>
      ...
      {expanded && (
        <div className="border-t border-mat-border p-5 space-y-4 animate-slide-up">
          {editing && (
            <div className="bg-mat-panel border border-mat-gold/30 p-4 space-y-3 mb-4">
          ...
          <SequenceGrid
            sequence={sequence}
            techniques={techniques}
            isEditing={editing && !sequence.coach_assignment_pending}
